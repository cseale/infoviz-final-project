...
const tip = d3.tip().attr('class', 'd3-tip').html(d => 'My Tooltip');
...
const projection = d3GeoProjection.geoMiller()
  .scale(160)
...
function ready([world, flowData]) {
  ...
  const flows = filterMigrationData(flowData.results);
...
export function deregisterMapClickHandler(onClick) {
  const index = clickHandlers.indexOf(onClick);
  if (index > -1) {
